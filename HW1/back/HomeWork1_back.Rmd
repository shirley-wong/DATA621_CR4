...
par(mfrow=c(3,2))   
for (i in 1:16) {
      hist(mtd[,i],main=names(mtd[i]),xlab=names(mtd[i]),51) # ? 51 mean(mtd$TARGET_WINS)
...
#  rename(`Variable Missing Data`=variable,`Number of Records`=n,`Share of Total`=percent) %>%
  ...
  kable_styling()
```
The varriable `BATTING_HBP` () hit by pitcher ) is missing over 90% of it's data. 
...
## Run the `step_back` model on Eval data.
```{r}
